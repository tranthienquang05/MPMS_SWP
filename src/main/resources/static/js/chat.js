(function () {
  ...
  function renderContacts() {
    if (contacts.length === 0) {
      contactListEl.innerHTML = '<div class="chat-empty">Không có ai để nhắn tin</div>';
      return;
    }
    contactListEl.innerHTML = contacts.map(c => {
      const roleLower = (c.role || '').toLowerCase();
      const preview = c.lastMessage ? escapeHtml(c.lastMessage) : 'Chưa có tin nhắn';
      const unread = c.unreadCount > 0;
      return '' +
        '<div class="chat-contact-item' + (c.id === activeContactId ? ' active' : '') + (unread ? ' unread' : '') + '" data-contact-id="' + c.id + '">' +
          '<div class="chat-contact-info">' +
            '<div class="chat-contact-name">' + escapeHtml(c.fullname) +
              ' <span class="badge-role badge-' + roleLower + '">' + escapeHtml(c.role) + '</span>' +
            '</div>' +
            '<div class="chat-contact-preview">' + preview + '</div>' +
          '</div>' +
          (unread ? '<div class="chat-contact-unread-badge">' + c.unreadCount + '</div>' : '') +
        '</div>';
    }).join('');

    contactListEl.querySelectorAll('.chat-contact-item').forEach(el => {
      el.addEventListener('click', () => openThread(el.getAttribute('data-contact-id')));
    });
  ...
  function sendMessage() {
    ...
    if (!content || !activeContactId || !stompClient || !stompClient.connected) return;
    stompClient.publish({
  ...
  function handleIncoming(message) {
    const isActiveThread = message.senderId === activeContactId || message.receiverId === activeContactId;
    ...
    if (contact) {
      ...
      if (message.receiverId === currentUserId && activeContactId !== partnerId) {
